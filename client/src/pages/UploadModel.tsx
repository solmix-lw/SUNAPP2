...
export default function UploadModelPage() {
  ...
  const { data: parts = [] } = useQuery<SparePart[]>({
    ...
    queryFn: async () => {
      ...
      return json.items as SparePart[];
    },
  });

  // Filter parts without a 3D model
  const partsWithoutModels = (parts ?? []).filter((p) => !p.model3dPath);
  ...
  const uploadMutation = useMutation({
    mutationFn: async (data: { partId: string; file: File }) => {
      ...
      const res = await fetch(`/api/parts/${data.partId}/upload-model`, {
        method: "POST",
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    const valid = [".glb", ".gltf", ".obj"];
    const ext = file.name.slice(file.name.lastIndexOf('.')).toLowerCase();
    if (!valid.includes(ext)) {
      toast({ title: "Invalid file type", description: "Please select a GLB, GLTF, or OBJ file", variant: "destructive" });
      return;
    }
  ...
  const handleUpload = () => {
    if (!selectedPartId) {
      toast({ title: "Error", description: "Please select a part first", variant: "destructive" });
      return;
    }
    if (!selectedFile) {
      toast({ title: "Error", description: "Please select a 3D model file", variant: "destructive" });
      return;
    }
    uploadMutation.mutate({ partId: selectedPartId, file: selectedFile });
  };
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex-none border-b bg-card p-6">
        ...
        <p className="text-muted-foreground mt-1">Add 3D schematics for spare parts</p>
      </div>
        ...
        <div className="max-w-2xl mx-auto space-y-6">
          ...
          {uploadStatus === "error" && (
            <Alert variant="destructive">
              ...
              <AlertDescription>Failed to upload the 3D model. Please try again.</AlertDescription>
            </Alert>
          )}
          <Card>
            <CardHeader>
                ...
                <Select value={selectedPartId} onValueChange={setSelectedPartId}>
                  ...
                  <SelectContent>
                    {partsWithoutModels && partsWithoutModels.length > 0 ? (
                      ...
                      <SelectItem value="none" disabled>No parts without models found</SelectItem>
                    )}
                  </SelectContent>
                </Select>
                <p className="text-xs text-muted-foreground">Only parts without existing 3D models are shown</p>
              </div>
              <div className="space-y-2">
                ...
                />
                <label htmlFor="model-file" className="border-2 border-dashed rounded-lg p-8 text-center hover-elevate cursor-pointer block">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">GLB, GLTF, or OBJ files (max 50MB)</p>
                    </div>
                  </div>
                </label>
                <p className="text-xs text-muted-foreground">Supported formats: .glb (recommended), .gltf, .obj</p>
              </div>
              <div className="pt-4">
                <Button className="w-full" onClick={handleUpload} disabled={!selectedPartId || uploadMutation.isPending} data-testid="button-upload">
